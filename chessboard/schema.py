...
def load_docs(schema_file):
    ...
    try:
        ...
        LOG.warning("Schema file '%s' is missing or not readable", DOCS_PATH)
        raise
...
#
DOCS = load_docs(open(DOCS_PATH, 'r'))
...
def parse_type_name(type_string):
    ...
    .. doctest::
        ...
        >>> print parse_type_name('string')
        ('string', None)
        >>> print parse_type_name('list(int)')
        ('list', 'int')
        >>> print parse_type_name('dict(option)')
        ('dict', 'option')
        >>> print parse_type_name('else(option)')
        ('else(option)', None)

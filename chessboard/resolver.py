...
class Planner(object):
    ...
    def _unique_providers(self):
        ...
        providers = []
        names = []
        for name, provider in self.environment.providers.items():
    ...
    def evaluate_defaults(self):
        ...
        """
        for option in self.blueprint.get('options', {}).values():
    ...
    def add_resources(self, context):
        ...
        # FIXME: see if we can handle this through dependency resolution
        pre_services = []
        post_services = []
        for key, details in self.blueprint.services.items():
    ...
    def add_resource_for_service(self, context, service_name, service_index):
        ...
        for resource in resources:
            ...
            for key in list(extra_components):
    ...
    def add_static_resources(self, deployment, context):
        ...
        for key, resource in blueprint.get('resources', {}).items():
    ...
    def connect_resource(self, resource, definition):
        ...
        """
        for key, connection in definition.get('connections', {}).items():
    ...
    def resolve_relations(self):
        """Identify source and target keys for all relations.

        We search for connection points in provides/requires/supports.

        Note: Assumes that find_components() has already run and identified all
        the components in the deployment. If not, this will effectively be a
        noop.
        """
        LOG.debug("Analyzing relations")
        services = self.deployment['blueprint'].get('services', {})
        coercer = schema.volup.Schema([schema.Relation()])
        for service_name, service in services.items():
    ...
    def resolve_recursive_requirements(self, context, history):
        ...
        for service_name, service in services.items():
            extra_components = service.get('extra-components')
            if not extra_components:
                continue
            for component_key, component in extra_components.items():
    ...
    def _satisfy_requirement(self, requirement, requirement_key, component,
        ...
        if not provides_match:
            raise exceptions.ChessboardValidationError(
                "Could not identify target for requirement '%s'" %
                requirement_key)
        info = {
    ...
    def _find_requires_key(relation, component):
        ...
        """
        for key, requirement in component.get('requires', {}).items():
    ...
    def _find_provides_key(relation, component):
        ...
        """
        for key, provided in component.get('provides', {}).items():
    ...
    def _find_supports_key(relation, component):
        ...
        """
        for key, connection_point in component.get('supports', {}).items():

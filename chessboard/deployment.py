...
class Deployment(classes.ExtensibleDict):
    ...
    def validate_options(self):
        ...
        Raise error if not.
        """
        for key, option in self.get('options', {}).items():
    ...
    def _get_setting_value(self, name):
        ...
        """
        if name and isinstance(name, six.string_types) and '/' in name:
            node = self
            for key in name.split("/"):
                if key in node:
                    try:
    ...
    def parse_constraints(constraints):
        ...
        for constraint in constraint_list:
            if len(constraint) == 1 and 'setting' not in constraint:
                # it's one key/value pair which is not 'setting':path
                # Convert setting:value to full constraint syntax
                parsed.append(dict(zip(('setting', 'value'),
                                       next(i for i in constraint.items()))))
            else:

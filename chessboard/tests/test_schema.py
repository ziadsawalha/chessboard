...
class TestSchemaDocs(unittest.TestCase):
    ...
    def test_repr(self):
        """Check that the repr shows the schema name."""
        schema = cb_schema.DocumentedSchema(str, name='foo')
        self.assertEqual(str(schema), '<DocumentedSchema foo>')

    def test_bad_file(self):
        """Test if schema file is bad."""
        with self.assertRaises(yaml_parser.ParserError):
            cb_schema.load_docs(six.StringIO(""":"""))

    def test_no_file(self):
        """Test if schema file is missing."""
        with self.assertRaises(IOError):
            cb_schema.load_file('/dev/null/fail')

    def test_generate_docs(self):
        """Check that docs are generated form supplied docs."""
        self.assertEqual(
